...
exports.addProduct = BigPromise(async (req, res, next) => {
    ...
    if (req.files) {
        for (let index = 0; index < req.files.photos.length; index++) {
            let result = await cloudinary.v2.uploader.upload(req.files.photos[index].tempFilePath, { folder: "t-shirt-products" });
            images.push({
                id: result.public_id,
...
exports.addReview = BigPromise(async (req, res, next) => {
    ...
    const alreadyReviewed = product.reviews.find(
        (individualReview) => individualReview.user.toString() === req.user._id.toString()
    );
    if (alreadyReviewed) {
